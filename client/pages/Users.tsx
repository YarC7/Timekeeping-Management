...
export default function Users() {
  ...
  const add = () => {
    ...
    setRows([...rows, { id, name: `New User ${id}`, email: `user${id}@company.com`, role: "Member" }]);
  };

  const exportRows = (format: "csv" | "json") => exportData(rows, "users", format);
      ...
      <Card>
        <CardHeader className="flex-row items-center justify-between">
          ...
          <div className="flex gap-2">
            ...
            <Button onClick={add}><Plus className="mr-2" />Add</Button>
          </div>
        </CardHeader>
        <CardContent>
          <Table>
