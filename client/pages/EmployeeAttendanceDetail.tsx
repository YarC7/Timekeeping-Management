...
export default function EmployeeAttendanceDetail() {
  ...
  const { data: employee, isLoading: loadingEmp } = employeesApi.useDetail(id || "");
  const from = format(startOfMonth(month), "yyyy-MM-dd");
  const to = format(endOfMonth(month), "yyyy-MM-dd");
  const { data: list = [], isLoading } = useAttendancesList({ dateFrom: from, dateTo: to });

  const rows = useMemo(() => (list || []).filter((a) => a.employee_id === id).sort((a,b) => (a?.date || "").localeCompare(b?.date || "")), [list, id]);
  ...
  const exportCSV = () => {
    ...
    exportData(data, `${employee?.full_name ?? "employee"}-${format(month, "yyyy-MM")}`, "csv");
  };

  return (
    <MainLayout title="Employee Attendance Detail">
      <div className="flex items-center gap-2 mb-4">
        <Link to="/attendance" className="text-primary hover:underline">Back to Attendance</Link>
      </div>
      ...
      <div className="flex flex-col md:flex-row md:items-center gap-4 mb-6">
        <div className="flex items-center gap-3">
          ...
          />
          <div>
            <div className="text-lg font-semibold">{employee?.full_name ?? ""}</div>
            <div className="text-sm text-muted-foreground">{employee?.position ?? ""}</div>
          </div>
        </div>
        <div className="md:ml-auto flex items-center gap-2">
          <Button variant="outline" size="icon" onClick={() => setMonth((d) => addMonths(d, -1))}>
            <ChevronLeft className="h-4 w-4" />
          </Button>
          <div className="min-w-[140px] text-center font-medium">{format(month, "MMMM yyyy")}</div>
          <Button variant="outline" size="icon" onClick={() => setMonth((d) => addMonths(d, 1))}>
              ...
              <TableBody>
                {rows.map((r) => (
                  <TableRow key={r.attendance_id}>
                    ...
                    <TableCell>{r.total_hours != null ? r.total_hours.toFixed(2) : '-'}</TableCell>
                    <TableCell>
                      {r.status === 'Present' && (
                        <Badge className="bg-green-500 text-white border-green-500">Present</Badge>
                      )}
                      {r.status === 'Late' && (
                        <Badge className="bg-orange-500 text-white border-orange-500">Late</Badge>
                      )}
                      {r.status === 'Absent' && (
                        <Badge className="bg-gray-300 text-gray-900 border-gray-300">Absent</Badge>
                      )}
                      {r.status === 'Leave' && (
                        <Badge className="bg-blue-500 text-white border-blue-500">Leave</Badge>
                      )}
